...
public class UserController {
    ...
    public ResponseEntity<?> registerUser(@Valid @RequestBody UsersDto usersDto, BindingResult result){
        // Validate passwords match
        userValidator.validate(usersDto,result);

        ResponseEntity<?> errorMap = mapValidationErrorService.MapValidationService(result);
        if(errorMap != null)return errorMap;

        Users newUsers = userService.saveUser(usersDto);
